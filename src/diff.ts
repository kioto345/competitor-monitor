import { PageMeta, PageChange, DiffResult, TrackField } from './types';

export function diff(prev: PageMeta[], curr: PageMeta[], track: TrackField[]): DiffResult {
  const prevMap = new Map<string, PageMeta>(prev.map(p => [p.url, p]));
  const currMap = new Map<string, PageMeta>(curr.map(p => [p.url, p]));
